...
// Define collections once, outside the handler
  ...
  try {
    ...
    if (token) {
      ...
      if (existingSession) {
        ...
        isNewSession = true;
        await createNewSession(sessionToken, ipHash);
    ...
    const recentIPVote = await votesCollection.findOne({
      ipHash,
      votedAt: { $gt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000) }, // 3 day cooldown
    });

    if (recentIPVote) {
      return res.status(400).json({
        message:
          "A vote has already been cast from this location recently. Please try again later.",
        cooldownRemaining: Math.ceil(
          (recentIPVote.votedAt.getTime() +
            3 * 24 * 60 * 60 * 1000 -
            Date.now()) /
            (60 * 1000)
        ), // minutes
    ...
    if (existingVote) {
      return res
        .status(400)
        .json({ message: "You have already voted with this session." });
    }
...
async function createNewSession(token: string, ipHash: string): Promise<void> {
  const now = new Date();
...
function setTokenCookie(res: NextApiResponse, token: string) {
  res.setHeader(
    "Set-Cookie",
    `voteToken=${token}; Path=/; HttpOnly; Max-Age=${
      3 * 24 * 60 * 60
    }; SameSite=Strict`
  );
}
...
//     const recentIPVote = await votesCollection.findOne({
//       ipHash,
//       votedAt: { $gt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000) } // 24 hour cooldown
//     });

//     if (recentIPVote) {
//       return res.status(400).json({
//         message: 'A vote has already been cast from this location recently. Please try again later.',
//         cooldownRemaining: Math.ceil((recentIPVote.votedAt.getTime() + 3 * 24 * 60 * 60 * 1000 - Date.now()) / (60 * 1000)) // minutes
//       });
//     }
